...
kubectl hpa status list [-A] [--sort-by health-score] [--min-score 60] [--filter scaling-limited]
kubectl hpa status list -A --problem
kubectl hpa status scan
...
- `--max-score <score>`: `--health-score` と同じ。health scoreが指定値以下のHPAだけ表示
- `--min-score <score>`: health scoreが指定値以上のHPAだけ表示
- `--color auto|always|never`: table出力の色。デフォルトは `auto` で、端末に出す場合はhealth scoreやConditionを色付け
...
- `--dashboard`: watch出力をコンパクトなターミナルダッシュボード形式で表示
...
- `status` / `analyze` / `watch` のHPA名補完は現在のnamespaceを使います。`-A` 指定時は `namespace/name` 形式で候補を返します。
...
| Kubernetes v1.30〜v1.34 | `autoscaling/v2` の基本status/condition/metric表示は互換想定。クラスタ固有のEvents文言や新しいbehavior fieldは差が出る可能性あり |
| Kubernetes v1.35.0 | 検証済み |
| kind上のmetrics-server v0.8.1 | 検証済み |
| custom/external metrics adapters | HPA statusに見える範囲で対応 |
| KEDA管理のHPA | KEDA風label/annotation/nameを検出し、ScaledObject確認を促す。ScaledObject本体の直接取得は将来対応 |
...
- [x] **Watchダッシュボード:** `--watch --dashboard` でヘルス、レプリカ差分、Condition、Metricを1画面に集約。
- [x] **バッチ分析機能:** `scan` / `list -A --problem` で問題のあるHPAを一括診断。
- [x] **Suggest/Fix機能:** `--suggest` / `--fix --apply` により、具体的なパッチ案と適用フローを表示。
- [x] **KEDA / Custom Metricsヒント:** KEDA管理らしいHPAとExternal/Object metricの欠落・stale疑いをHPA statusから診断。
- [ ] **KEDA API連携:** ScaledObjectやTriggerAuthenticationを直接取得する深掘り診断。
